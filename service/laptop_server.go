...
func NewLaptopServer(store LaptopStore, imageStore ImageStore, rating RatingStore) *LaptopServer {
	return &LaptopServer{
		LaptopStore: store,
		ImageStore:  imageStore,
		RatingStore: rating,
...
func (server *LaptopServer) RateLaptop(stream pb.LaptopService_RateLaptopServer) error {
	for {
		...
		req, err := stream.Recv()
		if err == io.EOF {
			log.Print("RateLaptop no more data")
			break
		}
		if err != nil {
			return logError(status.Errorf(codes.Unknown, "cannot receive stream request: %v", err))
		...
		fmt.Println("sending rate resp for", laptopID)
		err = stream.Send(res)
		if err != nil {
			fmt.Println("rate send resp", err)
